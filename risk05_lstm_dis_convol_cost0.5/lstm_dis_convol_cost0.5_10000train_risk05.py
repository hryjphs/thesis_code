...
def get_sim_path(M, freq, np_seed, num_sim, H=0, mu=0.05, vol=0.2, S=100, K=100, r=0, q=0):                                                                
    ...
    return un_price.astype(np.float32), bs_price.astype(np.float32), bs_delta.astype(np.float32)
...
def get_sim_path_sabr(M, freq, np_seed, num_sim, H =0, mu=0.05, vol=0.2, S=100, K=100, r=0, q=0, beta=1, rho=-0.4, volvol = 0.6, ds = 0.001):
    ...
    return a_price.astype(np.float32), bs_price.astype(np.float32), bs_delta.astype(np.float32), bartlett_delta.astype(np.float32)
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, cash_flow_flag=0, dg_random_seed=1, num_sim=500002, sabr_flag = False,
        continuous_action_flag=False, ticksize=0.5, init_ttm=5,history=5, trade_freq=0.2, num_contract=1, sim_flag = True,
        mu=0.05, vol=0.2, S=100, K=100, r=0, q=0, beta=1, rho=-0.4, volvol = 0.6, ds = 0.001, k=0.5):
    ...
    def step_profit_loss(self, action):
        ...
        # calculate period reward (part 1)
        changeduetotransaction = (price - current_price) * position - (np.abs(position - current_position) +0.01*(position - current_position)**2) * self.ticksize * multi_t 
        #reward1 = reward.copy()
        # if tomorrow is end of episode
        if self.t == self.num_period - 1:
            done = True
            reward1 = changeduetotransaction - (max(price - self.strike_price, 0) - current_option_price) * self.num_contract * 100 - (position + 0.01*position**2) * self.ticksize * multi_t   #liquidate option and stocks
            reward = reward1 - self.k / 2 * (reward1)**2
        else:
            done = False
            reward1 = changeduetotransaction - (option_price - current_option_price) * self.num_contract * 100
            reward = reward1 - self.k / 2 * (reward1)**2
        ...
        return self.state, reward, done, info, reward1    #also return the p&l
...
class Agent():
    def __init__(self, lr, gamma, n_actions, epsilon, batch_size, env,
        ...
        self.q_eval = DuelingDeepQNetwork(n_actions, fc0_dims, fc1_dims, fc2_dims, name='eval_dc05')
        self.q_next = DuelingDeepQNetwork(n_actions, fc0_dims, fc1_dims, fc2_dims, name='target_dc05')
...
def test(total_episode_test, env, agent, name, delta_flag=False, bartlett_flag=False):
        ...
        for i in range(total_episode_test):
            ...
            while not done:
                ...
                # a step
                observation, reward, done, info,pl = env.step(action)
                reward_store.append(reward)
                pl_store.append(pl)

            # get final utility at the end of episode, and store it.
            pl_T.append(pl_store)
            r_T.append(reward_store)
        ...
        return u_T_store, r_T,pl_T,cpl_T
...
# -*- coding: utf-8 -*-
    ...
    filename = 'dddqn_tf2_lstm_dc05_risk05.png'
    ...
    delta_u,delta_r, delta_pl, delta_cpl = test(total_episode_test = total_episode_test, env = env_test2, agent = agent, name='delta_dc05_risk05.csv', delta_flag=True, bartlett_flag=False)
    rl_u, rl_r,rl_pl, rl_cpl = test(total_episode_test = total_episode_test, env = env_test2, agent = agent, name='rl_dc05_risk05.csv', delta_flag=False, bartlett_flag=False)
    ...
    df1.to_csv('delta_dc05_r_risk05.csv', index=False, encoding='utf-8')
    ...
    df2.to_csv('rl_dc05_r_risk05.csv', index=False, encoding='utf-8')
    ...
    df3.to_csv('rl_dc05_cpl_risk05.csv', index=False, encoding='utf-8')    

    history = dict(zip(epi, delta_cpl))
    #name = os.path.join('history', name)
    df4 = pd.DataFrame(history,index=[0])
    df4.to_csv('delta_dc05_cpl_risk05.csv', index=False, encoding='utf-8')    
    ...
    df5.to_csv('rl_dc05_pl_risk05.csv', index=False, encoding='utf-8')        
    ...
    df6.to_csv('delta_dc05_pl_risk05.csv', index=False, encoding='utf-8')    
